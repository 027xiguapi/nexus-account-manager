...
import { codexProviderService } from '../services/ProviderService'
...
export function ProviderSelector({ open, onOpenChange, onSuccess }: ProviderSelectorProps) {
  const [selectedPreset, setSelectedPreset] = useState<CodexProviderPreset | null>(null)
  const [apiKey, setApiKey] = useState('')
  const [isApplying, setIsApplying] = useState(false)

  const groupedPresets = {
    official: codexProviderPresets.filter((p) => p.category === 'official'),
    cn_official: codexProviderPresets.filter((p) => p.category === 'cn_official'),
    aggregator: codexProviderPresets.filter((p) => p.category === 'aggregator'),
    custom: codexProviderPresets.filter((p) => p.category === 'custom'),
  }
  ...
  const handleApply = async () => {
    ...
    setIsApplying(true)
    try {
      await codexProviderService.applyProvider(selectedPreset, apiKey.trim())
      toast.success('Provider 配置已应用', {
        description: '请重启 Codex 使配置生效',
      })
      onSuccess?.()
      onOpenChange(false)
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>选择 Codex Provider</DialogTitle>
          <DialogDescription>
            选择一个供应商预设并输入 API Key，配置将写入 ~/.codex/auth.json
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          {selectedPreset && (
            <>
              ...
              {selectedPreset.config.env.OPENAI_BASE_URL && (
